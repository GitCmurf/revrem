...
commits without editing tracked ignore files. Linked worktrees use the common
repository's `.git/info/exclude`. Existing ignore files are checked by exact
line match so longer paths like `work/.revrem/runs/` do not suppress the root
`.revrem/runs/` entry.
