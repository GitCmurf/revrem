...
def _loop_config(tmp_path, **overrides):
    ...
    return runner_mod.LoopConfig(**base)
...
def _drive_loop(config, runner, tmp_path):
    ...
    """
    try:
        summary = runner_mod.run_loop(config, runner, clock=FakeClock(), identity=FakeRunIdentity())
    except runner_mod.RunLoopFailed as exc:
        summary = exc.summary
...
def _run_clear_path(tmp_path):
    runner = FakeRunner({"review": runner_mod.CommandResult([], 0, stdout=CLEAR_REVIEW_STDOUT)})
...
def _run_findings_path(tmp_path):
    runner = FakeRunner(
        {
            "review": runner_mod.CommandResult([], 0, stdout=FINDINGS_REVIEW_STDOUT),
            "exec": runner_mod.CommandResult([], 0, stdout="remediated\n"),
        }
    )
    return _drive_loop(_loop_config(tmp_path), runner, tmp_path)
...
def _run_budget_path(tmp_path):
    runner = FakeRunner(
        {"review": runner_mod.CommandResult([], 0, stdout=FINDINGS_REVIEW_STDOUT, tokens=100)}
    )
