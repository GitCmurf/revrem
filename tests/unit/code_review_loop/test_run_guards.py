...
def test_skips_if_not_commit_after_remediation():
    ...
    run_guards.assert_worktree_stable_before_remediation(config, ctx, engine_state, expected_head=None)
...
def test_raises_if_inside_work_tree_and_dirty(monkeypatch):
    ...
    monkeypatch.setattr(run_guards, "current_non_artifact_status_lines", lambda c, cx: [" M dirty.py"])
    
    with pytest.raises(RuntimeError, match="worktree changed during run before remediation"):
        run_guards.assert_worktree_stable_before_remediation(config, ctx, engine_state, expected_head="mock-head")
