...
def test_phase_card_timeout_overlay_shows_effective_default_when_unset(
    tmp_path: Path,
) -> None:
    repo = _repo(
        tmp_path,
...
def test_phase_card_timeout_overlay_formats_int_and_float_values(
    tmp_path: Path,
) -> None:
    repo = _repo(
        tmp_path,
...
def test_loop_state_accepts_plain_profile() -> None:
    ...
    assert tui_loop_state.loop_header_text(profile) == "LOOP PHASES"
