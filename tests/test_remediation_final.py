        ...
        triage=profiles.TriageConfig(
            ...
            routes={
                ...
                "frontier-thinking": profiles.TriageRouteConfig(
                    harness="reserved", model="fake-clear", fallback="midtier-coder"
                ),
            },
        ),
    )
...
def test_failed_checks_any_matches_exact_command():
                ...
                rule=(
                    profiles.TriageRoutingRule(
                        ...
                        when=profiles.TriageRoutingRuleWhen(
                            failed_checks_any=("pytest -q tests/foo.py",)
                        ),
    ...
    context = policy.RoutingContext(
        ...
        refactor_depth="atomic",
        module_count=1,
        failed_checks=("pytest -q tests/foo.py",),
        safety_signals=(),
    )
...
def test_missing_prompt_fragment_fails_routing(tmp_path):
    ...
    with pytest.raises(ValueError, match="could not be resolved or is untrusted"):
        prompts_composer.compose_remediation_prompt(tmp_path, triage_payload, resolved, "rev")
...
"""
    ...
    assert any(
        path.endswith("remediation-1-prompt.txt") for path in summary["artifact_paths"]["prompts"]
    )
    assert any(path.endswith("routing-1.json") for path in summary["artifact_paths"]["routing"])
    assert any(
        path.endswith("routing-outcome-1.json") for path in summary["artifact_paths"]["routing"]
    )
    ...
    outcome_schema = json.loads(
        files("code_review_loop")
        .joinpath("schemas/routing-outcome-v1.schema.json")
        .read_text(encoding="utf-8")
    )
...
def test_routed_remediation_prompt_preserves_pending_check_failures(tmp_path, monkeypatch):
    ...
    profile = profiles.Profile(
        ...
        triage=profiles.TriageConfig(
            ...
            routes={"midtier": profiles.TriageRouteConfig(harness="fake", model="fake-clear")},
        ),
    )
    config = LoopConfig(
        base="main",
    ...
    prompt = (tmp_path / "artifacts" / "remediation-2-prompt.txt").read_text(encoding="utf-8")
