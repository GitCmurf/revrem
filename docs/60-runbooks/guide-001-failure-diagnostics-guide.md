...
| Provider transport/DNS failure | `failure.reason=provider_network_unavailable`, stderr phrases such as `failed to lookup address information`, `could not resolve host`, `network is unreachable`, `no route to host`, or `http/request failed: error sending request for url` | Treat as a retryable provider/network interruption. Rerun or resume from the saved boundary after local network/DNS stabilizes; do not treat benign auth words inside a long transcript as credential evidence. |
