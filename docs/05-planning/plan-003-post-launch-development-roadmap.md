...
docops_version: '2.0'
area: planning
description: Prioritized hardening and feature roadmap after the first public RevRem launch, including ambitious showcase features for expert profiles and data distillation.
keywords:
- revrem
- roadmap
- hardening
...
This plan ranks candidate investments after the public launch, bridging necessary 
stabilization (PyPI distribution, diagnostics) with ambitious "showcase" capabilities 
(data distillation, specialized expert profiles) that prove the long-term value of the loop.
...
4. **Event-stream foundation & Data Distillation.** Build the event interface
   that lets CLI, Rich progress, run history, and TUI consume the same loop
   events. Crucially, leverage this structured event stream to introduce an
   opt-in indexed archive of issues and remediation diffs, transforming the
   tool into an engine for organizational learning and model post-training data.
5. **TUI Run Execution & Expert Profiles.** Launch the TUI to run real loops, 
   and bundle it with heavily tuned, specialized expert profiles (e.g., Security, 
   Performance) to immediately showcase the tool's versatility.
6. **Non-OpenAI backend adapters.** Defer real Claude/Gemini/opencode support
...
highest-value workflow feature. Data distillation and expert profiles will serve 
as the "showcase" capabilities that prove the tool's ambition while the shared 
execution contracts are hardened.
...
| Indexed remediation archive (data distillation) | High | Medium | Event stream / History schema | P1 |
| Specialized expert profiles | High | Low | Profiles schema | P1 |
...
## Milestone 4: Event Stream And Data Distillation
...
Extend this foundation to capture high-value organizational data.
...
- **Data Distillation:** Add an opt-in `--archive-diffs` flag to keep an indexed, 
  structured history of findings and their corresponding remediation diffs.
- Design the archive schema to support future ingestion for lessons-learned 
  dashboards or model fine-tuning (post-training data).
...
- Archiving mode produces a structured, parsable dataset mapping the original 
  review finding to the applied diff.
...
## Milestone 7: Public DevEx Expansion And Expert Profiles

### Goal
Make RevRem easier to evaluate, learn, and contribute to by providing 
compelling out-of-the-box personas and high-quality examples.

### Scope
- **Specialized Expert Profiles:** Ship deeply-tuned specialized profiles 
  out-of-the-box to showcase the engine's versatility beyond generic reviews:
  - Security Auditor
  - Performance Tuner
...
- Built-in expert profiles can be invoked directly (e.g., `revrem --profile security`).
...
1. **PR A: Public project cleanup.**
   README follow-up, branch/ruleset docs, issue templates, labels, Scorecard refresh.
2. **PR B: Packaging and TestPyPI.**
   Build checks, package metadata, TestPyPI publish workflow, installation docs.
3. **PR C: `revrem preflight`.**
   Target-repo doctor/preflight command with JSON output and diagnostic artifacts.
4. **PR D: Triage contract.**
   Structured triage artifacts, profile/CLI docs, and remediation handoff tests.
5. **PR E: Event sink & Data Distillation.**
   Typed loop events, progress renderer adapters, and the `--archive-diffs` indexed history logic.
6. **PR F: TUI run execution.**
   Real run launch/cancel/monitor using the event sink.
7. **PR G: Expert Profiles.**
   Bundle and document the specialized out-of-the-box profiles.
8. **PR H: Harness contract.**
   Fake harness, backend capability schema, and one candidate secondary adapter spike.
